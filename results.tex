...
Another effect that can only be seen at full output is burst noise coming from the Zener diode used as a reference. This manifest a sudden jumps in the output current. The \device{DgDrive-500-LN} \#14 tested is one of the earlier versions and contains an affected reference, which was discovered during those tests. This issue and its solution is discussed in more detail in the next section.
...
\begin{figure}[ht]
    ...
    \caption{Popcorn noise in different samples of the \device{LM399} over a \qty{24}{\hour} period as recorded by the scanner system discussed in this section.}

...
The supply filters applied in this design uses a passive LC filter for all rails, then a capacitance multiplier on the diode supply. The combined filter is shown in figure \ref{fig:laser_driver_dgdrive_filter}. The diode supply and the analog rail, which is tapped off before the capacitance multiplier, are then fed to a low noise post-regulator, an \device{LT3045} and its negative counterpart, the \device{LT3094}. Both regulators have excellent power supply ripple rejection (PSRR) out to at least \qty{1}{\MHz} of more than \num{e3}. At low frequency the PSRR is even higher and more than \num{e5} can be expected. This allows a combined PSRR of better than \num{e6} from low to high frequencies, even beyond \qty{1}{\MHz}.
...
Regarding the filter circuit shown in figure \ref{fig:laser_driver_dgdrive_filter} a few explaining words on the choice of components are in order before going to the measurement of the PSRR. Going back to equation \ref{eqn:transfer_function_lc_filter}, we saw, that the undamped second filter has excessive ringing at the cutoff frequency, because the filter poles are imaginary. To address this, there are several solutions. The most simple one is adding a damping element, either be in parallel to the capacitor or the inductor. In this case a damping element in parallel to the capacitor was chosen, because the capacitor is, from a designers point of view, on \textit{our} side and any effect arising from the modified output impedance can be taken care of. Again, the transfer function must be calculated.
\begin{align}
...
This is the transfer function of a 3\textsuperscript{rd} order filter. This type of filter was discussed by \citeauthor{input_filter_middlebrook} \cite{input_filter_middlebrook} (reprinted in \cite{input_filter_middlebrook_reprint1} and \cite{input_filter_middlebrook_reprint2}). \citeauthor{input_filter_middlebrook} showed, that there is an optimal value for the series resitance $R_d$ given a capacitance $C_d$ and the filter components $L_1$ and $C_1$. This optimal value has minimal gain peaking, hence a minimal quality factor $Q$ at the resonance freuency. The existance of such an optimal value can be easily understood from the fact, that if $R_d = \infty$ the resonance frequency is $\omega_0 = \frac{1}{\sqrt{L_1 C_1}}$ and in case $R_d = 0$ it is $\omega_1 = \frac{1}{\sqrt{L_1 \left(C_1 + C_d\right)}}$. In between $\omega_0$ and $\omega_1$, there is a lossy zone, where $R_d$ due to its lossy nature reduces $Q$, but at both ends $Q = \infty$, so there must be a minimum in between. By calculating, the minimum value of the transfer function at the point of resonance, \citeauthor{input_filter_middlebrook} found the following results:
\begin{align}
    R_0 &\coloneqq \sqrt{\frac{L_1}{C_1}}\\
    n &\coloneqq \frac{C_d}{C_1} \Rightarrow C_d = n C_1\\
    Q_{optimal} &= \sqrt{\frac{(4+3 n) (2+n)}{2 n^2 (4+n)}}\\
    R_d &= R_0 \cdot Q_{optimal}
\end{align}

% TODO: Derive the 2x / 10 K rule. This is prepared in 'Capacitor life doubles theory.pdf'.
From these equations, it can be seen, that the damping capacitor $C_d$ needs to be fairly large, depending on $n$. A critically damped system with $Q = \num{0.5}$ would be preferred, but this would require $n \approx 6$ making the $C_d$ prohibitively large. For this filter $n=4$ was chosen, so making the filter slightly underdamped with the following properties. First, A large, low resistance inductor capable of carrying at least \qty{1}{\A} was chosen. In this case a Coilcraft \device{MSS1210-125KEB}. Choosing a large inductor allows to choose a smaller capacitor. High reliability capacitor were chosen to give a long lifetime of the device. Chossing capacitors rated with a lifetime of \qty{5000}{\hour} at \qty{105}{\celsius} gives a lifetime of more than \qty{10}{\year}, when assuming an Arrhenius law with a doubling of the lifetime every \qty{10}{\Kelvin}. Apart from the design  life of the design, there are no special requirements for the capacitor as there is no large ripple current to be expected. The input power supply is supposed to be a filtered low noise supply and not the unfiltered output of a DC/DC regulator. So it is possible to maximize $L_1$ and choose a smaller $C_1$ since board space is limited.
...
Do note, that $R_d$ does include the equivalent series resistance (ESR) of $C_d$, so this must be subtracted from the final value of the damping resistor placed on the board. After choosing the values above, the filter was then simulated using LTSpice including the series resitance and parasitic parallel capacitance of $L_1$. The parasitic capcitance will induce some oszillation at the self resonant frequency of \qty{1}{\MHz} and then limit the useful attenuation to more than \num{e3}. At the \qty{170}{\kHz} discussed before the damping is about the same figure. This is an order of magnitude better than the filter used by \citeauthor{libbrecht_hall} and does not even include the regulators that follow.

...
\begin{document}
    \begin{circuitikz}[
        ...
        \begin{pgfonlayer}{foreground}
            ...
            %(vset) to[short, *-] ++(0,-0.5) to [open, v=$V_{set}$, voltage=straight, o-] ++(0,-4) to[short, o-] ++(1,0) node[op amp, anchor=+](opamp){U1}
            %(opamp.out) --++ (1,0) node[pigfete, anchor=gate](FET){Q1}
            %(FET.S) --++ (0,1) coordinate (Rs) to [R, a=$R_S$, i<_=$I_{out}$, *-] ++(0,2) |- (vset)
            %(opamp.-) -- ++(-0.5,0) |- (Rs)

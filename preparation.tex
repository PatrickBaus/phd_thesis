...
Therefore, if the deadtime is added at a frequency high enough, so that the input amplifier shows a white noise behaviour, the dead time will have no influence on the Allan variance. This is the reason, why the Keysight \device{3458A} uses averaging beyond integration times of \qty{10}{\plc}. The noise corner frequency of the instrument is \qty{0.5}{\Hz} \cite{3458A_noise_floor, sampling_with_3458A}, while \qty{10}{\plc} translates into \qty{5}{\Hz} or \qty{6}{\Hz}, depending on the line frequency, which is an order of magnitude higher than the corner frequency to make sure the noise floor is white.

Finally, \citeauthor{psd_to_adev} \cite{psd_to_adev} notes that for measurement durations or averaging time $T \gg T_0$ $\sigma_A^2(T) \to \sigma_A^2(\tau)$. The means that for higher frequencies, the Allan variance is not affected, because the noise is white, and for lower frequencies, where $T \gg T_0$ holds, the Allan variance is unaffected as well. If the multimeter is correctly configured, the dead time of the device has little effect on the Allan variance. This is further discussed in the next section.

\clearpage
\subsection{Example}
Using the results from the previous sections, it is possible to simulate a typical measurement sample containing white noise, flicker noise and random walk behaviour. This was done using the \textit{AllanTools} library \cite{allantools} to generate time domain data, which was then converted to a power spectrum using the algorithm of \citeauthor{welch} \cite{welch}. The full Python source is available at \cite{}. The time domain data was downsampled using the Largest-Triangle-Three-Buckets algorithm created by \citeauthor{lttb} \cite{lttb} from $2^{25}$ data points to $2000$ for faster plotting. This downsampling algorithm kept the overall shape, yet the edges are a little rougher. The full be regenerated using the source code given above. The power spectrum and the Allan deviation were always calculated from the original dataset.
...
\begin{figure}[ht]
    ...
    \begin{subfigure}{0.32\linewidth}
        ...
        \caption{Random walk}
    \end{subfigure}
    \caption{The three noise components summed together to simulate a typical noise source.}
    \label{fig:adev_example_noise_types}
\end{figure}

The the three time series shown in figure \ref{fig:adev_example_noise_types} were added together to give an impression how such a signal would look like. This is shown in figure \ref{fig:adev_example_time}. The summed series clearly shows the white noise content and it is possible to deduce some flicker or random walk noise, but it is highly obscured due to the amount of white noise. Using the time domain plot makes it very hard to distinguish the type of noise except for white noise.
...
A common approach to identify noise sources is the power spectrum. It is easily accessible using spectrum analyzers. The power spectrum of figure \ref{fig:adev_example_time} is shown in figure \ref{fig:adev_example_psd}. It allows identify a $f^{\alpha}$ component, but the $f^{-2}$ components is hardly recognizable. It can easily be mistaken for measurement noise, especially since there are fewer data points a low frequencies causing any fit to automatically favour the $f^{-1}$ part. For better a visualization the individual noise components are also plotted using dashed lines.
...
The Allan deviation plot gives very clean results and all noise components can be clearly identified. Again the individual components are plotted using dashed lines.
...
The Allan deviation was calculated using the overlapping Allan deviation algorithm \cite{oadev_definition}. The overlapping Allan deviation gives a better condifdence at longer intervals or lower frequencies, allowing to identify very low frequency noise like the random walk shown here. \cite{oadev_definition} also gives a very good comparison of other algorithms to identify noise types in data sets.

\section{Autozeroing}
Autozeroing, sometimes called zero-drift or dynamic offset compensation, is such an important concept, that it must be discussed in its own right. The need for autozeroing comes from the typical behaviour of an amplifier. Every amplifier shows some kind of offset and especially at high gains, this offset becomes a problem for high precision measurements. To make matters worse, this offset is not stable over time and drifts with both time and temperature. It can therefore not be calibrated out once, it must be permanently adjusted during operation. This procedure is called autozeroing.

There are many different ways to implement autozeroing and regarding operational amplifiers a good overview can be found in \cite{horowitz1989}. The most simple (digital) approach is to regularly switch the input from the signal to zero, take a reading, then subtract this reading from all subsequent readings until a new zero reading is taken. This approach is implemented in most modern DMMs and discussed first.

\subsection{Offset-Nulling}
In order to understand how offset-nulling works, espially how it shapes the spectrum, it is best to give an example. The example data set is a simulated measurement using a Keysight \device{3458A}. The corner frequency and the white noise floor is modeled after the \qty{10}{\V} range of the Keysight \device{3458A} \cite{3458A_noise_floor, sampling_with_3458A} mentioned above and the sample is generated using the Python \textit{AllanTools} library \cite{allantools}. The noise date is summed together, then added to a fixed \qty{10}{\V} offset, simulating measurement of a noise-free \qty{10}{\V} source. The sampling rate is chosen to be \qty{10}{\plc}, the same rate discussed previously on page \pageref{sec:dead_time}. In order to get a good low frequency estimate, $2^{24}=16.8 \cdot 10^{6}$ values were generated, the power spectrum was then averaged and binned to give \num{2000} datapoints.

The time domain plot of the simulated measurement is shown in figure \ref{fig:autozero_raw_time}. Both white noise and flicker noise are clearly visible.
...
To give a clearer picture of the noise spectrum, the power spectral density shown in figure \ref{fig:autozero_raw_psd} is calculated from the time series to confirm the flicker noise and white noise content. The \qty{0.5}{\Hz} corner frequency, which is defined as the intersection between the $f^{-1}$ noise and the white noise floor and the $\qty[power-half-as-sqrt, per-mode=symbol]{165}{\nV \Hz\tothe{-0.5}}$ noise floor can be easily identified.
...
Finally, it is possible to calculate the Allan deviation of the data set to discuss the application of the auto-zero algorithm. The Allan deviation plot is shown in figure \ref{fig:autozero_raw_adev}.

\begin{figure}[h]
    \centering
    \input{images/autozero_raw_adev.pgf}
    \caption{Allan deviation of a simulated measurement realized with a Keysight \device{3458A} containing white noise and flicker noise.}
    \label{fig:autozero_raw_adev}
\end{figure}

The Allan deviation shows two distinct regions, for the longer $\tau$ the flat flicker noise region can be identified, while for short $\tau$ an asymptotic behavour towards white noise with a $\tau^{−0.5}$ dependence can be seen. This plot allows to select the optimal offset-nulling frequency. As it was shown during the discussion of measurements with dead time it is preferable to add the dead time resulting from autozeroing at frequencies, that show pure, white noise, because this would not negatively impact the the measurement uncertainty. The interval, that shows mostly white noise is the $\tau = \qty{100}{\ms}$ interval, which is also the smallest interval possible at \qty{10}{\plc}.

While adding dead time has negligible influence on the Allan variance, the rest of the autozeroing algorithm needs to be discussed. The alternating measurement of the input, the zero and their subsequent subtraction has a noticable influence. In order to mabwechselndabwechselndathematically describe the effect, one assumption needs to be made. It is assumed, that the gain of input amplifier and ADC does not change over time. This assumption holds if the internal reference is stable and the ADC gain resistors are stable as well. If either of those two elements were to change, its drift cannot be corrected, as only the offset is measured. This would then introduce new $f^{-1}$ noise.

From figure \ref{fig:autozero_raw_adev} it can be seen, that both the signal measurement $x$ and the zero measurement $y$ are dominated by white noise. The regular variance of the combined measurement can then be calculated using equation \ref{eqn:adding_white_noise}:
\begin{equation}
    \sigma_{x-y}^2 = \sigma_x^2 + \sigma_y^2 \label{eqn:autozeroing}
\end{equation}

By subtracting the zero reading, the amplifier noise is effectively added twice to the final result, once for the input measurement and once for the zero measurement. Do note, that the number of samples is now half the number before applying autozeroing. This has another effect, that since the variance, or power, is doubled, but the number of samples halfed, the power spectral density also inscreased by another factor of $2$. This can be also be seen from equation \ref{}. To conclude, it is expected, that the variance doubles and the power spectral density quadruples!

Applying the autozeroing algorithm to the simulated data set, the constant \qty{10}{\V} input signal was nulled for every odd value and then the leftover noise was subtracted from its next lower neighbor. This results in the time domain are shown in figure \ref{fig:autozero_time}.
...
When comparing to figure \ref{fig:autozero_raw_time} it is immediately evident, that no $f^{-1}$ component is visible, but the white noise component has increased just as predicted by equation \ref{eqn:autozeroing}. To quantify this increase in white noise, the power spectral density can be used. Figure \ref{fig:autozero_psd}.

\begin{figure}[ht]
    \centering
    \input{images/autozero_psd.pgf}
    \caption{Simulated power spectrum of a Keysight \device{3458A} with autozeroing applied. The dashed lines denote the noise present before applying the autozero algorithm.}

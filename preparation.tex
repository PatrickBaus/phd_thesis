...
Laser diodes are current driven devices. The reason is that
\begin{equation}
    P_{out} \propto I
\end{equation}
...
$k_B$ is Boltzmann constant, $T$ the temperature, $q$ the electron charge and $V_d$ the diode voltage.

From the Shockley equation is clear, that a laser diode should be directly driven by a current and not a voltage to ensure precise operation.

The most basic function of a laser driver therefore is to provide a stable, but user adjustable current. Additional features like current and voltage limits, that shut down the diode aid in protecting the expensive laser diodes. The user adjustable current can typically be modulated at frequencies up to serveral \unit{\MHz} to shape the frequency and ampltitude of the laser beam. This section deals with the design challenges of such a device used for high precission laser spectroscopy.

A basic laser current driver design can be found in the work of \citeauthor{libbrecht_hall} \cite{libbrecht_hall}. While this design contains all the basic features, like a low noise current source, a modulation input and a voltage limit, there are several shortcomings that have emerged over the years with new generations of laser didoes. The laser driver used by legacy applications in this group is based on the aforementioned paper and has been successfully employed in several projects over the years, but several limitations have come up in recent years. In order to derive the design requirements of a new generation of laser drivers the important design elements need to be identified first. The essential design elements are the bulk current source, the modulation current source, the reference element and output programming. The next 4 sections will deal with each element and outline the design goals that were identified while employing the legacy generation of diode drivers in several experiments.
...
\begin{figure}[ht]
    ...
    \caption{An ideal current source with output impedance of $R_\mathrm{out}$.}

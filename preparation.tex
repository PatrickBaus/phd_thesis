...
Flicker noise is also called $\frac 1 f$-noise and it can be observed in many naturally occuring phenomenen and its origin is not clear, although there have been many explanations. An overview can be found in \cite{flicker_noise_overview, flicker_noise_overview2}. This work concentrates on flicker noise in electronic devices. For example in thick-film resistors it was shown to extend over at least 6 decades without any visible flattening \cite{1_f_noise_thick_film}. One explanation for flicker noise in transistors is the existance of generation-recombination noise or burst noise discussed in the previous section. If there are many uncorrelated trap sites, that contribute to the total noise, the envelope of the noise spectral density changes from $\frac{1}{f^2}$ to $\frac{1}{f^1}$. This is shown in figure \ref{fig:flicker_noise_evelope}
...
Mathematically this can be motivated using the assumption that the number of trap sites $N$ with a certain time constant $\tau$ is inversely proportional to the time constant:
\begin{equation}
    N(\tau) \propto \frac 1 \tau \label{eqn:flicker_noise_weight_function}
\end{equation}

This assumption is physical, because it is unreasonable, that a trap site can store an electron indefinitely and also it is reasonable that there is little correlation as trap sites spread over a large area in a semiconductor do not influence each other. Using equation \ref{eqn:burst_noise_lorentzian} from the previous section, multiplying the weight function \ref{eqn:flicker_noise_weight_function} and integrating over all possible storage times gives:
\begin{align}
    S(\omega) &= \lim_{t \to \infty} \int_0^t N(\tau) \, 4 R_{xx}(0) \frac{\tau}{1 + \omega^2 \tau^2} \, d\tau \nonumber\\
    &= 4 R_{xx}(0)\, C \lim_{t \to \infty} \int_0^t \frac{1}{1 + \omega^2 \tau^2} \, d\tau \nonumber\\
    &= \frac{4 R_{xx}(0)\, C}{\omega} \lim_{t \to \infty}  \arctan{\tau \omega} \Big|_0^t \nonumber\\
    &= \frac{4 R_{xx}(0)\, C}{\omega} \cdot \frac{\pi}{2} \nonumber\\
    &= \frac{2 \pi R_{xx}(0)\, C}{\omega}
\end{align}

$C$ is the proportionality constant and it can be seen, that for many distributed trap sites, a noise spectrum of $\frac{1}{f}$ is found.
...
\begin{figure}[ht]
    ...
    \end{subfigure}
    \caption{Different representations of flicker noise for different $\bar \tau_1$ and fixed $\bar \tau_0 = \qty{1}{\s}$.}

...
\end{equation}
It can be graphically extracted from the power spectral density plot by drawing a line trough the the flicker noise and the white noise and finding the intersection. This can be seen in figure \ref{fig:adev_example_psd} on page \pageref{fig:adev_example_psd}. The corner frequency can be found where the dashed blue and green line meet.
...
Consequently, if the deadtime is added at a frequency high enough, so that the input amplifier output is dominated by white noise, the dead time will have no influence on the Allan variance. This is the reason, why the Keysight \device{3458A} uses averaging beyond integration times of \qty{10}{\plc}. The noise corner frequency of the instrument is \qty{0.5}{\Hz} \cite{3458A_noise_floor, sampling_with_3458A}, while \qty{10}{\plc} translates into \qty{5}{\Hz} or \qty{6}{\Hz}, depending on the line frequency, which is an order of magnitude higher than the corner frequency to make sure the noise floor is (mostly) white.

Finally, \citeauthor{psd_to_adev} \cite{psd_to_adev} notes that for measurement durations or averaging times $T \gg T_0$, the Allan variance with respect to $T$ shows an asymptotic behaviour of $\sigma_A^2(T) \to \sigma_A^2(\tau)$. The means that for higher frequencies, the Allan variance is not affected, because the noise is white, and for lower frequencies, where $T \gg T_0$ holds, the Allan variance is unaffected as well. If the multimeter is correctly configured, the dead time of the device has little effect on the Allan variance. This is further discussed in the next section.

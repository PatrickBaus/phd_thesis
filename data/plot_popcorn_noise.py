...
def plot_series(plot):
  ...
  if not data.empty:
    ...
    plt.tick_params('x', labelbottom=False)
    plot_data(ax, data, plot_settings['columns_to_plot'][1], plot_settings['labels'], linewidth=0.5)

    ax = plt.subplot(313, sharex=ax1, sharey=ax1)
    plot_data(ax, data, plot_settings['columns_to_plot'][2], plot_settings['labels'], linewidth=0.5)
...
#  fig.set_size_inches(128/25.4 * 2.7 * 0.8, 96/25.4 * 1.5 * 0.8)  # Latex Beamer size 128 mm by 96 mm
  fig.set_size_inches(418.25555 / 72.27 * 0.9, 418.25555 / 72.27*(5**.5 - 1) / 2 * 0.9)
  ...
  plots = [
    {
      ...
      'zoom': ['2022-08-31 06:00:00', '2022-09-01 06:00:00'],  # popcorn noise comparison, used in PhD thesis
      ...
      'primary_axis': {
        ...
        'columns_to_plot': ["K2002 CH1", "K2002 CH9", "K2002 CH10"],  # popcorn noise comparison, used in PhD thesis

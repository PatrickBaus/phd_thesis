...
def main():
    ...
    D = 1/tau0*2**(0.5)  # normalized to the sample rate, similar to qd to make sure all ADEVs start at the same y-value
    # Generate noise: white (beta=-2), flicker (beta=-3), random-walk (beta=-4)
    betas = [-2, -3, -4]
    #betas = [-2,]
    labels = {  # betas
        -2: "White noise",  # mu = -1
        -3: "Flicker noise",  # mu = 0
        -4: "Random walk"  # 0 mu = 1
    }
    ...
    qd = {beta: adev0**2 / (normalization_coeffients[beta] * 2*(2*np.pi)**beta * tau0**(beta+1) * (2*np.pi)**2) for beta in betas}
    ...
    if "psd" in plots_to_show:
        ...
        for beta, (freqs, psd), ha in zip(betas, psds, has):
            lines, = ax.loglog(freqs[1:], [ha*pow(freq, beta+2) for freq in freqs[1:]],
                '--', label=f'$h_{{{beta+2}}}f^{{{beta+2}}}$', color=beta_colors[beta])
    ...
    phi = (5**.5-1) / 2 if plot_direction == "horizontal" else (5**.5+1) / 2  # golden ratio
    phi = 1
    scale = 0.3 * len(plots_to_show)
    scale = 0.9  # scale to 0.9 for (almost) full text width

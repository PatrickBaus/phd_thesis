...
def plot_series(plot):
  ...
  if not data.empty:
    ...
    if plot.get('secondary_axis', {}).get('show', True):
      ...
      prepare_axis(ax=ax2, fixed_order=plot_settings2.get('axis_fixed_order'), label=plot_settings2['label'], color_map=['firebrick', 'salmon',])
  ...
  plots = [
    {
      ...
      'zoom': ['2017-06-25 00:00:00', '2017-06-26 00:00:00'], # Serial no. ?, used in PhD thesis
      'crop_secondary_to_primary': False,
      'primary_axis': {
        'label': "Voltage deviation in V",
        'plot_type': 'relative',  # absolute, relative, proportional
        'axis_fixed_order': -6,
        'columns_to_plot': ["value",],  # popcorn noise comparison, used in PhD thesis
        'filter': None,

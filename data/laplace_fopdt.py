...
def calc_response(t, m, u):
    ...
    for i in range(ns):
        ...
        pv[i+1] = y[-1]
    return (pv[1:], op[1:]) # strip off the first value, which was used for the input step
...
pv2 = [model.k * (1.0 - np.exp(-(t[i] - model.theta) / model.tau)) for i in range(len(t))]

ax = plt.subplot(111)
plt.plot(t, pv2, linewidth=2, label=r'$1-e^{-\frac{t-\theta}{\tau} }$', alpha=0.7, linestyle='dashed')
plt.plot([t[0], model.theta, model.theta+0.0001, t[-1]],[0,0,1,1], linewidth=2, label=r'$H(t- \theta)$', alpha=0.7, linestyle='dotted')
